...
def myPca(X,num_components):
    ...
    X_projected = np.dot(eigenvector_subset.transpose(), X_meaned.transpose()).transpose()

    return Mean_col, Std_col, X_meaned, eigenvalue_subset, eigenvector_subset, X_projected


data = load_wine()

Mean_col, Std_col, X_meaned, eigenvalues, eigenvectors, X_projected = myPca(data.data,3)
...
reconstructed_X = X_projected@eigenvectors.T + Mean_col
...
ax.scatter(recon_class[:, 4], recon_class[:, 5], recon_class[:, 6], color='red', alpha=0.6, label='reconstructed data')
ax.scatter(original_class[:, 4], original_class[:, 5], original_class[:, 6], color='green', alpha=0.6, label='Original data')
